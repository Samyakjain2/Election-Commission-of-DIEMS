	...
	<div class="row">
		...
		<div class="col-md-9">
			<div class="callout callout-info" id="printable">
				<div>
					<h3 class="text-center">Election Result</h3>
					<hr>
					<table width="100%">
						<tr>
							<td width="50%">
							...
							<td width="50%">
								<p><b>Academic Year: <span id="ay"><?php echo $_SESSION['academic']['year'] ?></span></b></p>
							</td>
						</tr>
						<tr>
							<td width="50%">
					...
					<p class=""><b>Total Votes Casted: <span id="tse"></span></b></p>
				</div>
...
<script>
				...
				if (resp) {
					...
					if (Object.keys(resp).length <= 0) {
						...
						if (Object.keys(data).length > 0) {
							...
							var newRow = '<tr>';
							newRow += '<td colspan="20" class="text-left"><b>Average Percentage: </b>' + '&nbsp;'.repeat(132);
							for (var i = 0; i < averagePercentageString.length; i++) {
								if (averagePercentageString[i] === '%' && i === averagePercentageString.length - 1) {
									...
									newRow += '<b>' + averagePercentageString[i] + '</b>' + '&nbsp;'.repeat(16); // Add '%' with 17 spaces if it's not the last character
								} else {
	...
	function createPieChart(question, data) {
		...
		if (Array.isArray(data)) {
			...
			// Check if data is an object (new format)
			var pieChart = new Chart(ctx, {
				type: 'pie',
				data: {
